    ...
    for k, (train_index, val_index) in enumerate(kf.split(dataset)):
        ...
        for epoch in tqdm(range(args.start_epoch, args.epochs + 1), unit='epoch', leave=False):
            ...
            for _, (inputs, masks) in enumerate(loop):
                ...
                loss = segment_loss + l2_loss*(1-args.ssim_loss_rate) + ssim_loss*(args.ssim_loss_rate)
            ...
            logger.info(f'Train {k} [Epoch {epoch}/{args.epochs}] Train Loss : {seg_loss_list[-1]+l2_loss_list[-1]+ssim_loss_list[-1]} (Focal:{seg_loss_list[-1]}, L2:{l2_loss_list[-1]}, SSIM:{ssim_loss_list[-1]})')
            ...
            with torch.no_grad():
                ...
                for i, (inputs, masks) in enumerate(val_loop):
                    ...
                    if epoch == args.epochs:
                        out_mask = torch.argmax(out_masks,dim=1)
                        out_mask = out_mask.unsqueeze(1)
                        val_iou_list.extend(CalcIoU(masks, out_mask))
